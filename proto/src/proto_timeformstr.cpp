...
TEST_F(Proto_TimeFromStr, disect_string_to_ints)
{
  ...
  for (auto& piece : str_vec)
    int_vec.push_back(std::stoi(piece));
...
TEST_F(Proto_TimeFromStr, construct_time_type)
{
  ...
  std::tm tm{};
  std::istringstream ss(str_vec.at(0) + ":" + str_vec.at(1) + ":" +
    str_vec.at(2) + ":" + str_vec.at(3));
...
TEST_F(Proto_TimeFromStr, now_to_tm_struct)
{
  ...
  BOOST_LOG_TRIVIAL(trace) << std::put_time(
    std::localtime(&tm_time), "%c %Z");
}
...
TEST_F(Proto_TimeFromStr, now_to_tm_struct_and_change_time)
{
  ...
  if (std::round(std::stof(str_vec[3]) / 100) == 1)
    tm.tm_sec++;
